...
<div class="max-w-5xl mx-auto space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            ...
            <p class="text-slate-400 mt-1 font-medium">
                Evaluating <span class="text-white font-bold">{{ $learner->getFullName() }}</span> 
                <span class="mx-2 text-slate-700">|</span> 
                Brief: <span class="text-indigo-400 font-bold italic text-sm">{{ $brief->getTitle() }}</span>
            </p>
        </div>
    ...
    <form action="/trainer/evaluations/store" method="POST" class="space-y-6">
        <!-- Hidden Foreign Keys -->
        <input type="hidden" name="learner_id" value="{{ $learner->getId() }}">
        <input type="hidden" name="brief_id" value="{{ $brief->getId() }}">

        <!-- Skills Loop -->
        <div class="space-y-6">
            @forelse($briefSkills as $skill)
                <div class="bg-slate-800 rounded-3xl border border-slate-700 overflow-hidden shadow-2xl transition-all hover:border-indigo-500/30">
                    <div class="p-6 bg-slate-900/40 border-b border-slate-700 flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <span class="flex items-center justify-center w-10 h-10 bg-indigo-600/10 border border-indigo-500/20 rounded-xl text-indigo-400 font-black text-sm">{{ $skill->getCode() }}</span>
                            <h3 class="text-sm font-bold text-white uppercase tracking-wider">{{ $skill->getDescription() }}</h3>
                        </div>
                        <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">{{ $skill->getCategory()->value }}</span>
                    </div>
                    ...
                    <div class="p-8 grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                ...
                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="skills[{{ $skill->getId() }}][level]" value="IMITATE" class="peer hidden" required>
                                ...
                                <!-- ADAPT -->
                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="skills[{{ $skill->getId() }}][level]" value="ADAPT" class="peer hidden">
                                ...
                                <!-- TRANSPOSE -->
                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="skills[{{ $skill->getId() }}][level]" value="TRANSPOSE" class="peer hidden">
                        ...
                        <!-- Feedback Comment -->
                        <div class="space-y-4">
                            <label for="comment_{{ $skill->getId() }}" class="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Pedagogical Comment</label>
                            <textarea id="comment_{{ $skill->getId() }}" name="skills[{{ $skill->getId() }}][comment]" rows="3" placeholder="Provide constructive feedback on technical execution..." 
            ...
            @empty
                <div class="bg-slate-800 rounded-3xl border border-slate-700 border-dashed p-10 text-center">
                    <p class="text-slate-400 font-medium">No competencies linked to this brief yet.</p>
                </div>
            @endforelse
        </div>
        ...
        <div class="bg-slate-800 p-8 rounded-3xl border border-slate-700 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
            ...
            <div class="flex items-center space-x-4 w-full md:w-auto">
                <button type="button" class="px-6 py-3 text-slate-500 font-bold hover:text-white transition-colors text-xs uppercase tracking-widest" onclick="history.back()">Discard</button>
